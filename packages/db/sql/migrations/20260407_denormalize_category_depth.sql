...
SET
    category_depth = ac.depth
FROM public.amazon_categories ac
WHERE
    ac.id = pcr.category_id
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS product_category_ranks_bs_canonical_idx ON public.product_category_ranks (
    asin,
    category_depth ASC,
    rank DESC
)
