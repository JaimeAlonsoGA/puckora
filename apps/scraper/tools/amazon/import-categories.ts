 ...
 *   npx ts-node src/import-categories.ts ./data/amazon-browse-nodes.xlsx
 *   npx ts-node src/import-categories.ts ./data/amazon-browse-nodes.xlsx --dry-run
 *
...
async function main() {
  ...
  const filePath = args.find(a => !a.startsWith('--')) ?? './data/amazon-browse-nodes.xlsx'
